...
class TestValidFormat:
    def test_standard_15_char(self) -> None:
        ...
        assert result["data"]["rto_code"] == "02"
        assert result["data"]["year_of_issue"] == "2019"
    ...
    def test_delhi_dl(self) -> None:
        ...
        assert result["success"] is True
        assert result["data"]["valid"] is True
        assert result["data"]["state_code"] == "DL"
        assert result["data"]["state_name"] == "Delhi"
...
class TestEdgeCases:
    def test_dl_unknown_state_code(self) -> None:
        ...
        assert result["data"]["state_name"] == "Unknown"

    def test_dl_future_year(self) -> None:
        result = validate_driving_license("MH9921991234567")
        assert (
            "out of plausible range" in str(result["errors"]).lower()
            or result["data"]["valid"] is False
        )
    ...
    def test_dl_hyphenated_input(self) -> None:
        ...
        assert result["success"] is True
        assert result["data"]["valid"] is True
        assert "-" not in result["data"]["dl_number"]
