...
class TestRealCIN:
    def test_reliance_cin(self) -> None:
        ...
        assert result["data"]["listing_status"] == "Listed"
        assert result["data"]["nic_code"] == "17110"
        assert result["data"]["state_code"] == "MH"
        assert result["data"]["state_name"] == "Maharashtra"
        assert result["data"]["year_of_incorporation"] == "1973"
        assert result["data"]["company_type_code"] == "PLC"
        assert result["data"]["company_type"] == "Public Limited Company"
        assert result["data"]["sequential_number"] == "019786"
...
class TestListingStatus:
    ...
    def test_unlisted(self) -> None:
        ...
        assert result["success"] is True
        assert result["data"]["valid"] is True
        assert result["data"]["listing_status"] == "Unlisted"
...
class TestCompanyTypes:
    ...
    def test_ptc(self) -> None:
        result = validate_cin("U72200MH2007PTC175696")
        assert result["data"]["company_type"] == "Private Limited Company"

    def test_flc(self) -> None:
        result = validate_cin("U72200MH2007FLC175696")
        assert result["data"]["company_type"] == "Foreign Company"

    def test_goi(self) -> None:
        result = validate_cin("U72200MH2007GOI175696")
        assert result["data"]["company_type"] == "Government Company"

    def test_npl(self) -> None:
        result = validate_cin("U72200MH2007NPL175696")
        assert result["data"]["company_type"] == "Not for Profit License Company"

    def test_opc(self) -> None:
        result = validate_cin("U72200MH2007OPC175696")
        assert result["data"]["company_type"] == "One Person Company"

    def test_ulc(self) -> None:
        result = validate_cin("U72200MH2007ULC175696")
        assert result["data"]["company_type"] == "Unlimited Liability Company"
...
class TestRegexFail:
    def test_21_chars_but_bad_format(self) -> None:
        result = validate_cin("X17110MH1973PLC019786")
        assert result["success"] is False
        assert result["data"]["valid"] is False
